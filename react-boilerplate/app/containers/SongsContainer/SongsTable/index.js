...
class SongSorter extends React.Component {
  ...
  handleSongSelect = e => {
    if (e.target.checked) {
      this.setState(
        {
          selectedSongs: this.state.selectedSongs.concat(
      ...
      this.setState(
        {
          selectedSongs: this.state.selectedSongs.filter(id => id !== parseInt(e.target.id, 10)),
        },
        () => this.handlePlaylistButtonToggle()
      );
    }
  ...
  handlePlaylistButtonToggle = () => {
    if (this.state.selectedSongs.length < 1) {
  ...
  render() {
    ...
    return (
      <div>
          ...
          <Table.Header className="stickyTop">
            <Table.Row>
              ...
              <Table.HeaderCell>
                ...
                <Icon
                  name="sort"
                  onClick={() => this.handleSort('songs.title')}
                />
              </Table.HeaderCell>
              <Table.HeaderCell>
                Uploader
                <Icon
                  name="sort"
                  onClick={() => this.handleSort('songs.owner')}
                />
              </Table.HeaderCell>
              <Table.HeaderCell>
                Rating
                <Icon
                  name="sort"
                  onClick={() => this.handleSort('songratings.rating')}
                />
              </Table.HeaderCell>
              <Table.HeaderCell>
                Genre
                <Icon
                  name="sort"
                  onClick={() => this.handleSort('songs.genres')}
                />
              </Table.HeaderCell>
              <Table.HeaderCell>
                Date Uploaded
                <Icon
                  name="sort"
                  onClick={() => this.handleSort('songs.dateUploaded')}
                />
              </Table.HeaderCell>
            </Table.Row>
          </Table.Header>
              ...
              {window.location.pathname === '/stream' ? (
                <Table.Cell collapsing>
                  <Paginator
                    ...
                    onSetCurrentItem={this.props.onSetCurrentItem}
                  />
                </Table.Cell>
              ) : (
                <Table.Cell />
        ...
        {this.props._loading ? <Loader active size="massive" /> : <div />}
      </div>
    );
  }
}
