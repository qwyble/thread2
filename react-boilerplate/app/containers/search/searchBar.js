...
class SearchBar extends React.Component {
  ...
  handleSearchChange = e => {
    this.setState({ _loading: true, searchString: e.target.value }, () => {
      axios({
        method: 'get',
        url: `https://thread-204819.appspot.com/getAllSearch/${escape(
          this.state.searchString || '%'
        )}`,
      }).then(result => {
        const users = result.data.users.slice(0, 4);
        const playlists = result.data.playlists.slice(0, 4);
        const songs = result.data.songs.slice(0, 4);
